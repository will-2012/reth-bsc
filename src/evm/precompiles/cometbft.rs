...
fn decode_light_block_validation_input(input: &[u8]) -> DecodeLightBlockResult {
    ...
    // Decode the protobuf‐encoded `LightBlock` that follows the consensus-state section. We
    // switched from the deprecated `merge` API – which expects a length-delimited field – to
    // the more robust `decode` helper that can parse the full buffer directly.  This improves
    // compatibility with newer `prost` releases and avoids false negatives on valid inputs.

    let light_block_bytes = &input[CONSENSUS_STATE_LENGTH_BYTES_LENGTH as usize + cs_length as usize..];

    let light_block_pb = match TmLightBlock::decode(light_block_bytes) {
        Ok(pb) => pb,
        Err(_) => return Err(BscPrecompileError::CometBftInvalidInput.into()),
    };
